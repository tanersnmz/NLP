...
class FeatureExtractor(object):
    ...
    def get_input_representation(self, words, pos, state):
        ...
        top_stack = ['NULL', 'NULL', 'NULL']
        top_buffer = ['NULL', 'NULL', 'NULL']

        if len(state.stack) > 0:
            top_stack[0] = state.stack[-1]
        if len(state.stack) > 1:
            top_stack[1] = state.stack[-2]
        if len(state.stack) > 2:
            top_stack[2] = state.stack[-3]
            
        if len(state.buffer) > 0:
            top_buffer[0] = state.buffer[-1]
        if len(state.buffer) > 1:
            top_buffer[1] = state.buffer[-2]
        if len(state.buffer) > 2:
            top_buffer[2] = state.buffer[-3]

        combined_representation = top_stack + top_buffer
        input_vector = np.zeros(6)
        i = 0
        while i < 6:
            element = combined_representation[i]           
            if element != 'NULL':
                pos_tag = pos[element]
            else:
                pos_tag = 'NULL'           
            if pos_tag in pos_mapping:
                input_vector[i] = pos_mapping[pos_tag]
            else:
                if element != 'NULL':
                    word = words[element].lower()
                else:
                    word = ''                   
                if word in self.word_vocab:
                    input_vector[i] = self.word_vocab[word]
                else:
                    input_vector[i] = pos_mapping['UNK']            
            i += 1
        return np.array(input_vector)

    def get_output_representation(self, output_pair):  
        vec_output = np.zeros(91)
        vec_output[self.output_labels[output_pair]] = 1
        return vec_output
